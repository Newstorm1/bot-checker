...
$MadelineProto = new \danog\MadelineProto\API('turbo.madeline',$settings);
require("conf.php"); 
...
if($from_id == $sudo){
try{
if(file_get_contents("stepTurbo") == "2"){
if($text !== "Login Turbo"){
  file_put_contents('phonetr',$text);
$MadelineProto->phonelogin($text);
$tg->vtcor('sendmessage',[
'chat_id'=>$chat_id, 
'text'=>"⌁ TuRbo (1) - \n⌁ Send Code Phone Now \n⌁ Ex : - 67467 -",
]);
file_put_contents("stepTurbo","3");
}
}elseif(file_get_contents("stepTurbo") == "3"){
if($text){
...
file_put_contents("stepTurbo","4");
}else{
$tg->vtcor('sendmessage',[
'chat_id'=>$chat_id, 
'text'=>"⌁ TuRbo (1) - \n⌁ Done Login ✅",
]);
file_put_contents("stepTurbo","");
exit;
}
}
}elseif(file_get_contents("stepTurbo") == "4"){
if($text){
